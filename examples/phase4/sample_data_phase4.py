"""sample_data_phase4.py - サンプルデータ生成"""
import pandas as pd
import numpy as np
...
series_ids = ['S001', 'S002', 'S003', 'S004']
...
for sid in series_ids:
    ...
    for i, date in enumerate(dates):
        data.append({
            'unique_id': sid,
            'ds': date,
            'y': base + trend[i] + seasonal[i] + noise[i],
            'y_scaled': (base + trend[i] + seasonal[i] + noise[i] - 60) / 20,
            'price': np.random.uniform(5.0, 20.0),
            'promo': np.random.choice([0, 0, 0, 0.1, 0.2]),
            'region': np.random.choice(['Kanto', 'Kansai', 'Chubu'])
        })

df = pd.DataFrame(data)

# 保存
output_dir = Path('examples/phase4')
